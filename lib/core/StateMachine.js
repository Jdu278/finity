...
class StateMachine {
  ...
  async handleUnhandledEvent(event, eventPayload) {
    if (this.config.global.unhandledEventHooks.length > 0) {
      return (await (0, _invokeEach2.default)(this.config.global.unhandledEventHooks, event, this.currentState, this.createContextWithEvent(event, eventPayload)))[0];
    }
  ...
  async executeTransition(transitionConfig, context) {
    ...
    const nextState = transitionConfig.targetState !== null ? transitionConfig.targetState : this.currentState;

    await (0, _invokeEach2.default)(this.config.global.transitionHooks, this.currentState, nextState, context);
    const actionRetvals = await (0, _invokeEach2.default)(transitionConfig.actions, this.currentState, nextState, context);
  ...
  async enterState(state, context) {
    await (0, _invokeEach2.default)(this.config.global.stateEnterHooks, state, context);

    const stateConfig = this.config.states[state];
    if (stateConfig) {
      await (0, _invokeEach2.default)(stateConfig.entryActions, state, context);
    }

    if (this.currentState !== null && this.currentState !== state) {
      await (0, _invokeEach2.default)(this.config.global.stateChangeHooks, this.currentState, state, context);
    }
  ...
  async exitState(context) {
    ...
    await (0, _invokeEach2.default)(this.config.global.stateExitHooks, this.currentState, context);

    const stateConfig = this.config.states[this.currentState];
    if (stateConfig) {
      await (0, _invokeEach2.default)(stateConfig.exitActions, this.currentState, context);
    }
  }
  ...
  startAsyncAction(asyncActionConfig, state, context) {
    this.taskScheduler.enqueue(() => {
      ...
      this.taskScheduler.enqueue(async () => {
        action(state, context).then(result => handleComplete(successTrigger, { result }), error => handleComplete(failureTrigger, { error }));
      }).then(null, x => {
        throw x;
      });
  ...
  async getFirstAllowedTransitionForEvent(context) {
    ...
    if (eventConfig) {
      transitionConfig = await StateMachine.getFirstAllowedTransition(eventConfig.transitions, context);
    }

    if (!transitionConfig && stateConfig.anyEventTrigger) {
      transitionConfig = await StateMachine.getFirstAllowedTransition(stateConfig.anyEventTrigger.transitions, context);
    }
  ...
  async executeTrigger(triggerConfig, context) {
    return await this.taskScheduler.enqueue(async () => {
      const transitionConfig = await StateMachine.getFirstAllowedTransition(triggerConfig.transitions, context);
      if (transitionConfig) {
        return await this.executeTransition(transitionConfig, context);
      }
      return undefined;
